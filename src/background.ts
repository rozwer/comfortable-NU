...
async function setupCalendarSyncAlarm() {
  return new Promise<void>((resolve, reject) => {
    chrome.storage.local.get(['calendarSyncInterval', 'autoSyncEnabled', 'userAuthenticatedExplicitly'], (result) => {
      ...
      chrome.alarms.clear(CALENDAR_SYNC_ALARM_NAME, () => {
        if (autoSyncEnabled && isAuthenticated) {
          ...
          logger.debug(`アラームをクリアしました (autoSync=${autoSyncEnabled}, authenticated=${isAuthenticated})`);
        }
        resolve();
      });
    });
...
async function shouldAutoSync(): Promise<boolean> {
  return new Promise((resolve) => {
    chrome.storage.local.get(['lastSyncTime', 'calendarSyncInterval', 'autoSyncEnabled', 'userAuthenticatedExplicitly'], (result) => {
      if (chrome.runtime.lastError) {
      ...
      if (chrome.runtime.lastError || !token) {
        ...
        verifyTokenSecurity(token)
          .then(() => {
            ...
            setStorageDirect({ 'userAuthenticatedExplicitly': true }).then(() => {
  ...
  return new Promise((resolve) => {
    ...
    chrome.storage.local.get(['userAuthenticatedExplicitly'], (result) => {
      ...
      chrome.identity.getAuthToken({ interactive: false }, async (token) => {
        ...
        if (!token) {
          logger.debug('[DEBUG] No existing token (cache may have been cleared), returning empty without clearing auth flag');
